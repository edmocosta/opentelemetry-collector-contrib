...
func main() {
	...
	err = writeGoFile("tables.go", "localizedtime", []byte(codeWriter.String()))
	if err != nil {
		log.Fatal(err)
	}
}
		...
		for _, monthContext := range gregorianCalendar.Months.MonthContext {
			...
			for _, monthWidth := range monthContext.MonthWidth {
				if monthWidth.Type == "abbreviated" {
					...
					if err != nil {
						return fmt.Errorf("failed to read %s short month names %v", tag, err)
					}
					...
					if err != nil {
						return fmt.Errorf("failed to read %s long month names %v", tag, err)
					}
				}
			}
		}
...
func writeGoFile(filename, pkg string, b []byte) error {
	...
	header += "// Code generated by running \"go generate\" in open-telemetry/opentelemetry-collector-contrib/internal/coreinternal/timeutils/internal/localizedtime. DO NOT EDIT.\n\n"
